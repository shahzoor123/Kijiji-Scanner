...
# \\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\
    ...
    def checking(self):
        ...
        # Scrape new ads
        new_ads = self.scrape_data(url_to_scrape_actual_data)
        ...
        if not os.path.exists(seen_ads_file):
            ...
            if unseen_ads:
                ...
                with open(seen_ads_file, 'a') as file:
                    file.write('\n'.join(unseen_ads))
                    file.write('\n')
                    self.Send_SMS(url_to_scrape_actual_data, unseen_ads)
